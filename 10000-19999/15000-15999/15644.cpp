...
queue<pair<Marble, string>> q;
pair<Marble, string> answer = {Marble(0, 0, 0, 0), ""}, p = {Marble(0, 0, 0, 0), ""};
bool can_exit = false;

void MOVE(Marble marble, string path, char direction){ // 효율적으로 못 짰음ㅎㅎ;;
	Marble temp = marble;
	...
	if (direction == 'L'){
		...
		if (temp != marble){q.push({marble, path + "L"});}
		return;
	}
	else if (direction == 'R'){
		if (marble.red_marble_x > marble.blue_marble_x){
		...
		if (blue_exit == true){return;}
			...
			can_exit = true;
			answer = {marble, path + "R"};
			return;
		}
		if (visit[marble.red_marble_y][marble.red_marble_x][marble.blue_marble_y][marble.blue_marble_x] == true){return;} // 이미 이 배치가 있었던 적이 있다면
		visit[marble.red_marble_y][marble.red_marble_x][marble.blue_marble_y][marble.blue_marble_x] = true;
		if (temp != marble){q.push({marble, path + "R"});}
		return;
	}
	else if (direction == 'U'){
		...
		if (blue_exit == true){return;}
			...
			can_exit = true;
			answer = {marble, path + "U"};
			return;
		}
		if (visit[marble.red_marble_y][marble.red_marble_x][marble.blue_marble_y][marble.blue_marble_x] == true){return;} // 이미 이 배치가 있었던 적이 있다면
		visit[marble.red_marble_y][marble.red_marble_x][marble.blue_marble_y][marble.blue_marble_x] = true;
		if (temp != marble){q.push({marble, path + "U"});}
		return;
	}
	else if (direction == 'D'){
		if (marble.red_marble_y > marble.blue_marble_y){
		...
		if (blue_exit == true){return;}
			...
			can_exit = true;
			answer = {marble, path + "D"};
			return;
		}
		if (visit[marble.red_marble_y][marble.red_marble_x][marble.blue_marble_y][marble.blue_marble_x] == true){return;} // 이미 이 배치가 있었던 적이 있다면
		visit[marble.red_marble_y][marble.red_marble_x][marble.blue_marble_y][marble.blue_marble_x] = true;
		if (temp != marble){q.push({marble, path + "D"});}
		return;
	}
}
...
int main(){
	...
	q.push({marble, ""});
	visit[marble.red_marble_y][marble.red_marble_x][marble.blue_marble_y][marble.blue_marble_x] = true;
	while (!q.empty() && move_count < 10 && can_exit == false){
		move_count++;
		qsize = q.size();
		while (qsize-- > 0 && can_exit == false){
			p = q.front();
			q.pop();
			MOVE(p.first, p.second, 'L');
			MOVE(p.first, p.second, 'R');
			MOVE(p.first, p.second, 'U');
			MOVE(p.first, p.second, 'D');
		}
	}
