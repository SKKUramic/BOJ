...
// 18185 라면 사기 (Small)의 응용판
#include <stdio.h>

long long ramen[1000010] = {0, };
long long cost = 0, ramen_count = 0;
int N;

long long ret_3min(long long a, long long b, long long c){
	if (a < b){
		if (a < c) {return a;}
		else {return c;}
	}
...
long long ret_2min(long long a, long long b){
	if (a < b){return a;}
	else {return b;}
}

int main(){
	long long one_cost, two_cost, three_cost, B, C, two_buy, three_buy;
	...
	for (int n = 0; n < N; n++){ // 현재의 공장에서 얼마나 살지 곧바로 결정한다
		...
		if (ramen[n + 1] > ramen[n + 2]){ 
			...
			cost += two_cost * two_buy; // ramen[n + 2]가 0이라면 비용 계산은 여기까지다
			ramen[n] -= two_buy;
			ramen[n + 1] -= two_buy;
			three_buy = ret_3min(ramen[n], ramen[n + 1], ramen[n + 2]);
			cost += three_cost * three_buy;
			ramen[n] -= three_buy;
			ramen[n + 1] -= three_buy;
			ramen[n + 2] -= three_buy;
		}
			...
			cost += two_cost * two_buy;
			ramen[n] -= two_buy;
			ramen[n + 1] -= two_buy;
		}
		cost += one_cost * ramen[n]; // 낱개 구매, 이후 더이상 ramen[n]의 값을 참조하지 않는다
	}
	printf("%lld", cost);
}